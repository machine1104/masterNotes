...
{
    puts("Usage: loramac <initialize|start|get|set|join|tx|link_check"
...
static int _cmd_loramac(int argc, char **argv)
{
   ...
   /* START LOOP DATA SENDING  */
        ...
        printf ("Setting deveui, appeui and appkey for Device1\n");

        uint8_t deveui[LORAMAC_DEVEUI_LEN];
        fmt_hex_bytes(deveui, "00F1B2F1D57B1942");
        semtech_loramac_set_deveui(&loramac, deveui);

        uint8_t appeui[LORAMAC_APPEUI_LEN];
        fmt_hex_bytes(appeui, "70B3D57ED002D548");
        semtech_loramac_set_appeui(&loramac, appeui);

        uint8_t appkey[LORAMAC_APPKEY_LEN];
        fmt_hex_bytes(appkey, "02CF29303865A74E16E850ECEEFFB443");
        semtech_loramac_set_appkey(&loramac, appkey);
    }
    else if (strcmp(argv[1], "start") == 0) {
        
        
        printf ("Listening to the nature...\n");
