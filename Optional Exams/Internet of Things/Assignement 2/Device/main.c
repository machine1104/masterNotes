...
static int cmd_pub(int argc, char **argv)
{
    ...
    printf("Published '%s' on topic '%s' ", argv[2], argv[1]);
...
static int cmd_start(int argc, char **argv)
{
	...
	while(i!=5){
		...
		printf("%d",i);
		const char m[] = "{'value':SIMULATED_VALUE}";
		if (emcute_pub(&t, m, strlen(m), flags) != EMCUTE_OK) {
			printf("error: unable to publish data to topic '%s [%i]'\n",
				t.name, (int)t.id);
		return 1;
		}
...
static const shell_command_t shell_commands[] = {
    ...
    { "start","simulate device",cmd_start },
    { NULL, NULL, NULL }
};

int main(void)
{
    puts("Device simulator on RIOT OS\n");
